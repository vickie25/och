-- Complete Financial Module Migration - 100% Implementation
...
CREATE INDEX CONCURRENTLY idx_revenue_metrics_latest ON revenue_metrics(metric_type, revenue_stream, calculated_at DESC);
CREATE INDEX CONCURRENTLY idx_financial_alerts_active ON financial_alerts(status, created_at DESC) WHERE status = 'active';
CREATE INDEX CONCURRENTLY idx_cash_flow_latest ON cash_flow_projections(projection_type, created_at DESC);
CREATE INDEX CONCURRENTLY idx_compliance_overdue ON compliance_records(next_review_date) WHERE next_review_date < CURRENT_DATE;

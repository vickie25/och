...
from .models import Cohort, Enrollment, ProgramRule, Waitlist
    ...
    for enrollment in enrollments:
        try:
            ...
            if evaluation['meets_criteria']:
                ...
                # Enforce certificate eligibility gates (payments, missions, etc).
                from programs.services.certificate_eligibility_service import (
                    CertificateEligibilityService,
                )
                is_eligible, details = CertificateEligibilityService.check_eligibility(enrollment)
                if not is_eligible:
                    # Roll back completion if gates not met.
                    ...
                    completed_count -= 1
                    logger.info(
                        "Enrollment %s not eligible for completion/certificate: %s",
                        enrollment.id,
                        details.get('summary'),
                    )
                    continue

                # Issue certificate (creates + generates DOCX if possible)
                ...
                try:
                    ...
                    logger.error("Certificate issuance error for enrollment %s: %s", enrollment.id, e)
            else:
                enrollment.status = 'incomplete'
                enrollment.save()
                incomplete_count += 1

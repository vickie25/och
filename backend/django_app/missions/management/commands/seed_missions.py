...
class Command(BaseCommand):
    help = 'Seed missions with complete OCH mission data'
    ...
    def handle(self, *args, **options):
        ...
        for mission_data in missions_data:
            ...
            if created:
                self.stdout.write(self.style.SUCCESS(f'Created mission: {mission.code} - {mission.title}'))
            else:
                self.stdout.write(self.style.WARNING(f'Updated mission: {mission.code} - {mission.title}'))

        self.stdout.write(self.style.SUCCESS(f'Successfully processed {len(missions_data)} missions'))

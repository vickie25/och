...
"""
    ...
    try:
        ...
        print(f"\n--- [DIRECT PSQL REPAIR ON REMOTE: {remote_host}] ---")
        client.exec_command(f"echo \"{sql}\" > /tmp/remote_repair.sql")
        
        # Execute psql to the remote host
        command = f"PGPASSWORD=postgres psql -h {remote_host} -U postgres -d ongozacyberhub < /tmp/remote_repair.sql"

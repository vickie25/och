...
from users.models import CustomUser, DeviceTrust

try:
    print('Testing CustomUser...')
    u = CustomUser.objects.filter(email='mystartupkenya@gmail.com').first()
...
def execute():
    try:
        ...
        print("--- Writing Diagnostic Script ---")
        write_cmd = f"cat << 'EOF' > /tmp/diag_db.py\n{script}\nEOF"
        c.exec_command(write_cmd)
        
        run_sudo("docker cp /tmp/diag_db.py hub_prod_django:/app/diag_db.py")
        print("--- Executing Diagnostic Script ---")
        out, err = run_sudo("docker exec hub_prod_django python /app/diag_db.py")
        print("OUTPUT:\\n", out)
        if err: print("ERR:\\n", err)

...
def execute():
    try:
        ...
        out, _ = run_sudo("docker exec hub_prod_django python manage.py showmigrations users")
        print(out)
        
        print("Checking postgres tables ... ")
        out, _ = run_sudo("docker exec hub_prod_postgres psql -U ongoza -d ongozacyberhub -c '\\dt'")
        print(out)

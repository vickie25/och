...
def remote_repair_via_container():
    host = "69.30.235.220"
    user = "administrator"
...
"""
    ...
    try:
        ...
        print(f"\n--- [REPAIRING REMOTE DB VIA CONTAINER PSQL] ---")
        # Save SQL to a file that can be read by the container
        client.exec_command(f"echo \"{sql}\" > /tmp/remote_repair_final.sql")
        
        # Execute psql from WITHIN the local hub_prod_postgres container pointing to the remote IP
        command = f"sudo docker exec -i hub_prod_postgres psql -h {remote_host} -U postgres -d ongozacyberhub -c \"{sql}\""
        ...
        # ALSO: Fix the LOCAL password just in case the app refuses to move
        print("\n--- [ENSURING LOCAL DB PASSWORD IS 'postgres'] ---")
        command = "sudo docker exec hub_prod_postgres psql -U postgres -c \"ALTER USER postgres WITH PASSWORD 'postgres';\""
        stdin, stdout, stderr = client.exec_command(f'printf "%s\\n" "{password}" | sudo -S -p "" {command}', get_pty=True)

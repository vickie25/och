  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        setPreview(data)
        if (data?.contract?.type && data.contract.type !== 'employer') {
          setError('This link is for an employer contract. Use the institution onboarding link for institutional contracts.')
          return
        }
    ...
    try {
      ...
      const loginRes = await fetch('/api/auth/login', {
        ...
        body: JSON.stringify({
          email: result.email,
          password: form.password,
          device_fingerprint: `employer-onboarding-${Date.now()}`,
          device_name: 'Employer Onboarding',
        }),
      })
      if (!loginRes.ok) {
      ...
      const nextUrl = result.next || `/dashboard/employer?organization=${organization}&contract=${contract}`
  ...
  return (
    <div className="min-h-screen bg-och-midnight flex items-center justify-center p-6">
      <Card className="w-full max-w-3xl p-8 bg-och-midnight border border-och-steel/20">
        <h1 className="text-2xl font-bold text-white mb-3">Employer onboarding</h1>
        <p className="text-och-steel mb-6">
          Review your employer contract, set your account password, then continue to the employer portal for talent
          pipeline access, retainer billing, and placement workflows.
        </p>
        ...
        {loading ? (
          ...
          <>
            <div className="space-y-2 text-sm mb-6">
              ...
              <p className="text-och-steel">Type: <span className="text-white">{preview?.contract?.type || 'employer'}</span></p>
            ...
            <div className="rounded-lg border border-och-steel/20 bg-och-steel/10 p-4 text-sm text-och-steel mb-6">
              ...
              <p>1) Review contract and retainer / placement terms</p>
              <p>2) Complete employer profile and billing contacts</p>
              <p>3) Access talent pipeline and marketplace tools</p>
            </div>

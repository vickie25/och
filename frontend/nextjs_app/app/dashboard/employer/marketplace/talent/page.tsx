...
export default function TalentBrowsePage() {
  ...
  const loadTalent = async () => {
    try {
      ...
      if (filters.min_readiness) params.min_readiness = parseInt(filters.min_readiness, 10)
      ...
      const msg = err instanceof Error ? err.message : 'Failed to load talent profiles'
      setError(msg)
    } finally {
      setLoading(false)
    }
  ...
  const handleInterest = async (
    profileId: string,
    action: 'favorite' | 'shortlist' | 'contact_request'
  ) => {
    try {
      await marketplaceClient.logInterest(profileId, action)
    } catch (err: unknown) {
      console.error('Failed to log interest:', err)
      const msg = err instanceof Error ? err.message : 'Failed to save interest'
      alert(msg)
    }
  }
      ...
      <div className="max-w-7xl mx-auto">
        <div className="mb-6">
          <Link
            ...
            className="text-sm font-medium text-och-mint hover:text-och-mint/80 mb-3 inline-block"
          >
            ← Back to marketplace
          </Link>
              ...
              <div className="flex items-end">
                <label className="flex items-center gap-2 cursor-pointer">
                  <input
                    ...
                    onChange={(e) =>
                      setFilters((prev) => ({ ...prev, contactable_only: e.target.checked }))
                    }
                    className="w-4 h-4"
                  />
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {commonSkills.map((skill) => (
                  <button key={skill} type="button" onClick={() => toggleSkill(skill)} className="cursor-pointer">
                    <Badge variant={selectedSkills.includes(skill) ? 'gold' : 'defender'}>{skill}</Badge>
                  </button>
                ))}
              </div>
            </div>

...
export async function GET(
  ...
  try {
    ...
    const res = await fetch(
      `${FASTAPI_URL}/api/v1/profiling/enhanced/session/${sessionId}/value-statement`,
      {
        method: 'GET',
        headers: {
    ...
    if (!res.ok) {
      return NextResponse.json(
        { error: text || 'Failed to fetch value statement' },
        { status: res.status }
      )
    }
    ...
    console.error('[profiling/enhanced/session/value-statement] Proxy error:', err?.message || err)
    return NextResponse.json(
